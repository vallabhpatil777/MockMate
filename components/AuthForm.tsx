      ...
      async function onSubmit(values: z.infer<typeof formSchema>) {
        try{
            ...
            if (type === 'sign-in'){
                ...
                toast.success(`Signed in successfully as ${values.email}`)
                router.push('/')
            }
            else{
              ...
              if(!result?.success){
                ...
                toast.success(`Account created for ${values.name}. Please sign in.`)
                router.push('/sign-in')
            }
        } catch(err){
            console.log(err)
  ...
  return (
    <div className="card-border lg:min-w-[566px]">
        <div className="flex flex-col gap-6 card py-14 px-10">
            <div className="flex flex-row gap-2 justify-center"><Image src={logo} alt="logo" height={32} width={38}/><h2 className="text-primary-100">MockMate</h2></div>
